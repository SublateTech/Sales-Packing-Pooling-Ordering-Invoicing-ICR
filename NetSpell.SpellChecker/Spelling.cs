...
using Signature.SpellChecker.Forms;
using Signature.SpellChecker.Dictionary;
using Signature.SpellChecker.Dictionary.Affix;
using Signature.SpellChecker.Dictionary.Phonetic;
...
namespace Signature.SpellChecker
{
	...
	/// </summary>
    [ToolboxBitmap(typeof(Signature.SpellChecker.Spelling), "Spelling.bmp")]
	public class Spelling : System.ComponentModel.Component
	{
			...
			for (int i = 0; i < this.CurrentWord.Length; i++)
			{
				...
				for (int x = 0; x < tryme.Length; x++)
				{
					...
					if (this.TestWord(tempWord.ToString())) 
					{
                        ...
                        ws.EditDistance = Levenshtein.LevenshteinValue(this.CurrentWord, tempWord.ToString()); //this.EditDistance(this.CurrentWord, tempWord.ToString());
                        ws.Type = SuggestionType.BadChar;
			...
			if (this.CurrentWord.Length > 1) 
			{
				for (int i = 0; i < this.CurrentWord.Length; i++)
				{
					...
					if (this.TestWord(tempWord.ToString())) 
					{
						...
						//ws.EditDistance = this.EditDistance(this.CurrentWord, tempWord.ToString());
                        ws.EditDistance = Levenshtein.LevenshteinValue(this.CurrentWord, tempWord.ToString()); //this.EditDistance(this.CurrentWord, tempWord.ToString());
                        ws.Type = SuggestionType.ExtraChar;
			...
			for (int i = 0; i <= this.CurrentWord.Length; i++)
			{
				for (int x = 0; x < tryme.Length; x++)
				{
					...
					if (this.TestWord(tempWord.ToString())) 
					{
						...
						//ws.EditDistance = this.EditDistance(this.CurrentWord, tempWord.ToString());
                        ws.EditDistance = Levenshtein.LevenshteinValue(this.CurrentWord, tempWord.ToString()); //this.EditDistance(this.CurrentWord, tempWord.ToString());
                        ws.Type = SuggestionType.ForgotChar;
						tempSuggestion.Add(ws);
					}
				}			 
			}
			...
			for (int i = 0; i < replacementChars.Count; i++)
			{
				...
				while (pos > -1)
				{
					...
					if (this.TestWord(tempWord))
					{
                        ...
                        ws.EditDistance = Levenshtein.LevenshteinValue(this.CurrentWord, tempWord.ToString()); //this.EditDistance(this.CurrentWord, tempWord.ToString());
                        ws.Type = SuggestionType.ReplaceChars;
		...
		private void SwapChar(ref ArrayList tempSuggestion)
		{
			for (int i = 0; i < this.CurrentWord.Length - 1; i++)
			{
				...
				if (this.TestWord(tempWord.ToString())) 
				{
					...
					//ws.EditDistance = this.EditDistance(this.CurrentWord, tempWord.ToString());
                    ws.EditDistance = Levenshtein.LevenshteinValue(this.CurrentWord, tempWord.ToString()); //this.EditDistance(this.CurrentWord, tempWord.ToString());
                    ws.Type = SuggestionType.SwapChar;
		...
		private void TwoWords(ref ArrayList tempSuggestion)
		{
			for (int i = 1; i < this.CurrentWord.Length - 1; i++)
			{
				...
				if (this.TestWord(firstWord) && this.TestWord(secondWord)) 
				{
					...
					//ws.EditDistance = this.EditDistance(this.CurrentWord, tempWord.ToString());
                    ws.EditDistance = Levenshtein.LevenshteinValue(this.CurrentWord, tempWord.ToString()); //this.EditDistance(this.CurrentWord, tempWord.ToString());
                    ws.Type = SuggestionType.TwoWords;
		...
		public void Suggest()
		{
            ...
            */
			...
			{
				Word word = ((Word)tempSuggestion[i]);
