...
namespace OSAS
{
	...
	{ 
		...
		public OSAS_Orders(String CompanyID, String CustomerID)
		{
		...
		public void insert_detail_sql()
		{
			...
			Sql = String.Format("Insert into OrderDetail (Teacher, Student, Seq, ProductID, Quantity, Tax, QuantityInvoiced, CustomerID, CompanyID, Reserved )  Values (\"{0}\",\"{1}\",\"{2}\",'{3}','{4}','{5}','{6}','{7}','{8}','0')",
				this.Detail.Teacher,this.Detail.Student,this.Detail.Seq,this.Detail.Item,this.Detail.No_Items,this.Detail.Tax,this.Detail.No_Invoiced,this.Detail.CustomerID,this.Detail.CompanyID);
		...
		{
			Sql = String.Format("Update OrderDetail  Set  Seq=\"{0}\", ProductID='{1}', Quantity='{2}', Tax='{3}', QuantityInvoiced='{4}', Reserved='0'"+" Where CompanyID=\""+this.Header.CompanyID+"\""+" And CustomerID=\""+this.Header.CustomerID+"\""+" And Teacher=\""+this.Header.Teacher+"\""+" And Student=\""+this.Header.Student+"\" And ProductID=\""+this.Detail.Item+"\"",
				this.Detail.Seq,this.Detail.Item,this.Detail.No_Items,this.Detail.Tax,this.Detail.No_Invoiced);
