...
namespace Signature.Forms
{
	...
	/// </summary>
	public class frmViewStudent : System.Windows.Forms.Form
    {
        ...
        public frmViewStudent(String CompanyID, String CustomerID, String Teacher)
		{
			//
		...
		private void frmCustomerView_Load(object sender, System.EventArgs e)
		{
			...
			if (this.CustomerID.Trim() == "")
                dt = oMySql.GetDataView("Select distinct(Student) as Name, Teacher, sum(Nro_Items)as NoItems, CustomerID from Orders Where CompanyID='" + this.CompanyID + "' Group by Student,Teacher", "Students");
            else if (this.Teacher.Trim() == "")
                dt = oMySql.GetDataView("Select distinct(Student) as Name, Teacher, sum(Nro_Items)as NoItems, CustomerID from Orders Where CompanyID='" + this.CompanyID + "' And CustomerID='" + this.CustomerID + "' Group by Student,Teacher", "Students");
            else
                dt = oMySql.GetDataView("Select distinct(Student) as Name, Teacher, sum(Nro_Items)as NoItems, CustomerID from Orders Where CompanyID='" + this.CompanyID + "' And CustomerID='" + this.CustomerID + "' And Teacher='" + this.Teacher + "' Group by Student,Teacher", "Students");
            ...
            if (sender == txtSearch)
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (Grid.Rows.Count > 0)
                    {
                        ...
                        {
                            SelectedID = Grid.ActiveRow.Cells["Name"].Text;
